...
namespace KneeInnovation3D.EntityTools
{
    ...
    public class MeshBuilder
    {
        ...
        /// The positions.
        /// </summary>
        private IList<Point3D> _positions;
        ...
        /// </summary>
        private IList<int> _triangleIndices;
        ...
        {
            this._positions = new List<Point3D>();
            this._triangleIndices = new List<int>();

            if (generateNormals)
            {
                this._normals = new List<Vector3D>();
            }

            if (generateTextureCoordinates)
            {
                this._textureCoordinates = new List<Point>();
            }
        }
        ...
        public bool CreateNormals
        {
            ...
            set
            {
                if (value && this._normals == null)
                {
                    this._normals = new Vector3DCollection();
                }
        ...
        public bool CreateTextureCoordinates
        {
            get
            {
                return this._textureCoordinates != null;
            }

            set
            {
                if (value && this._textureCoordinates == null)
                {
                    this._textureCoordinates = new PointCollection();
                }
        ...
        public void AddCubeFace(Point3D center, Vector3D normal, Vector3D up, double dist, double width, double height)
        {
            ...
            {
                this._textureCoordinates.Add(new Point(1, 1));
                this._textureCoordinates.Add(new Point(0, 1));
                this._textureCoordinates.Add(new Point(0, 0));
                this._textureCoordinates.Add(new Point(1, 0));
            }

            this._triangleIndices.Add(i0 + 2);
            this._triangleIndices.Add(i0 + 1);
            this._triangleIndices.Add(i0 + 0);
            this._triangleIndices.Add(i0 + 0);
            this._triangleIndices.Add(i0 + 3);
            this._triangleIndices.Add(i0 + 2);
        }
        ...
        public void AddCylinderWithCap(Point3D p1, Point3D p2, double diameter, int thetaDiv)
        {
            Vector3D n = p2 - p1;
            double l = n.Length;
            n.Normalize();
        ...
        public void AddExtrudedGeometry(IList<Point> points, Vector3D xaxis, Point3D p0, Point3D p1)
        {
            ...
            int index0 = this._positions.Count;
            int np = 2 * points.Count;
            foreach (var p in points)
            {
                var v = (xaxis * p.X) + (ydirection * p.Y);
                this._positions.Add(p0 + v);
                this._positions.Add(p1 + v);
                v.Normalize();
                if (this._normals != null)
                {
                    this._normals.Add(v);
                ...
                this._triangleIndices.Add(i1);
                this._triangleIndices.Add(i2);
                this._triangleIndices.Add(index0);

                this._triangleIndices.Add(i1);
                this._triangleIndices.Add(i3);
                this._triangleIndices.Add(i2);
            }
        }
        ...
        {
            var indices = EntityTools.CuttingEarsTriangulator.Triangulate(points);
            var index0 = this._positions.Count;
            foreach (var p in points)
            {
                this._positions.Add(origin + (axisX * p.X) + (axisY * p.Y));
            }

            foreach (var i in indices)
            {
                this._triangleIndices.Add(index0 + i);
            }
        }
        ...
        public void AddExtrudedSegments(IList<Point> points, Vector3D axisX, Point3D p0, Point3D p1)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                this._positions.Add(p0 + d);
                this._positions.Add(p1 + d);
                ...
                if (this._textureCoordinates != null)
                {
                    ...
                    this._textureCoordinates.Add(new Point(0, v));
                    this._textureCoordinates.Add(new Point(1, v));
                }
            }
            ...
            IList<IList<Point>> textureCoordinateList)
        {
            ...
            for (int i = 0; i < positionsList.Count; i++)
            {
                ...
                foreach (var p in pc)
                {
                    this._positions.Add(p);
                }

                // add normals
                if (this._normals != null && normalList != null)
                {
                    var nc = normalList[i];
                ...
                // add texcoords
                if (this._textureCoordinates != null && textureCoordinateList != null)
                {
                    var tc = textureCoordinateList[i];
                    foreach (var t in tc)
                    {
                        this._textureCoordinates.Add(t);
                    }
                }
            }
        ...
        public void AddNode(Point3D position, Vector3D normal, Point textureCoordinate)
        {
            ...
            if (this._textureCoordinates != null)
            {
                this._textureCoordinates.Add(textureCoordinate);
            }
        }
        ...
        public void AddQuad(Point3D p0, Point3D p1, Point3D p2, Point3D p3, Point uv0, Point uv1, Point uv2, Point uv3)
        {
            ...
            if (this._textureCoordinates != null)
            {
                this._textureCoordinates.Add(uv0);
                this._textureCoordinates.Add(uv1);
                this._textureCoordinates.Add(uv2);
                this._textureCoordinates.Add(uv3);
            }
            ...
            IList<Point3D> quadPositions, IList<Vector3D> quadNormals, IList<Point> quadTextureCoordinates)
        {
            ...
            if (this._normals != null && quadNormals == null)
            {
                throw new ArgumentNullException("quadNormals");
            }

            if (this._textureCoordinates != null && quadTextureCoordinates == null)
            {
            ...
            if (this._textureCoordinates != null && quadTextureCoordinates != null)
            {
                ...
                {
                    this._textureCoordinates.Add(tc);
                }
            }
            ...
            int indexEnd = this._positions.Count;
            for (int i = index0; i + 3 < indexEnd; i++)
            {
                this._triangleIndices.Add(i);
                this._triangleIndices.Add(i + 1);
                this._triangleIndices.Add(i + 2);

                this._triangleIndices.Add(i + 2);
                this._triangleIndices.Add(i + 3);
                this._triangleIndices.Add(i);
            }
        }
            ...
            Point3D[,] points, Point[,] texCoords = null, bool closed0 = false, bool closed1 = false)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                {
                    this._positions.Add(points[i, j]);
                }
            }
            ...
            if (this._textureCoordinates != null)
            {
                if (texCoords != null)
                {
                    for (int i = 0; i < rows; i++)
                    {
                        for (int j = 0; j < columns; j++)
                        {
                            this._textureCoordinates.Add(texCoords[i, j]);
                        }
                    }
                }
                else
        ...
        public void AddRegularIcosahedron(Point3D center, double radius, bool shareVertices)
        {
            ...
            if (shareVertices)
            {
                ...
                {
                    this._positions.Add(center + (v * radius));
                }
        ...
        {
            if (this._textureCoordinates != null && textureValues == null)
            {
            ...
            for (int i = 0; i < thetaDiv; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    if (this._textureCoordinates != null)
                    {
                        this._textureCoordinates.Add(new Point((double)i / (thetaDiv - 1), textureValues == null ? (double)j / (n - 1) : textureValues[j]));
                    }
                }
            }
        ...
        public void AddRevolvedGeometry(IList<Point> points, IList<double> textureValues, Point3D origin, Vector3D direction, int thetaDiv)
        {
            ...
            for (int i = 0; i < thetaDiv; i++)
            {
                ...
                for (int j = 0; j + 1 < n; j++)
                {
                    ...
                    {
                        this._textureCoordinates.Add(new Point((double)i / (thetaDiv - 1), textureValues == null ? (double)j / (n - 1) : textureValues[j]));
                        this._textureCoordinates.Add(new Point((double)i / (thetaDiv - 1), textureValues == null ? (double)(j + 1) / (n - 1) : textureValues[j + 1]));
                    }
        ...
        {
            int p0 = this._positions.Count;
            this.Append(GetUnitSphere(subdivisions));
            int p1 = this._positions.Count;
            for (int i = p0; i < p1; i++)
            {
                this._positions[i] = center + (radius * this._positions[i].ToVector3D());
            }
        }
            ...
            for (int pi = 0; pi <= phiDiv; pi++)
            {
                ...
                for (int ti = 0; ti <= thetaDiv; ti++)
                {
                    ...
                    if (this._textureCoordinates != null)
                    {
                        ...
                        this._textureCoordinates.Add(uv);
                    }
                }
            }
        ...
        public void AddTriangleFan(IList<int> vertices)
        {
            ...
            {
                this._triangleIndices.Add(vertices[0]);
                this._triangleIndices.Add(vertices[i + 1]);
                this._triangleIndices.Add(vertices[i + 2]);
            }
        }
            ...
            IList<Point3D> fanPositions, IList<Vector3D> fanNormals = null, IList<Point> fanTextureCoordinates = null)
        {
            ...
            if (this._normals != null && this._normals == null)
            {
                throw new ArgumentNullException("fanNormals");
            }

            if (this._textureCoordinates != null && this._textureCoordinates == null)
            {
            ...
            if (this._textureCoordinates != null && fanTextureCoordinates != null)
            {
            ...
            if (this._normals != null && fanNormals != null)
            {
            ...
            IList<Point> stripTextureCoordinates = null)
        {
            ...
            if (this._normals != null && stripNormals == null)
            {
                throw new ArgumentNullException("stripNormals");
            }

            if (this._textureCoordinates != null && stripTextureCoordinates == null)
            {
            ...
            {
                this._positions.Add(stripPositions[i]);
                if (this._normals != null && stripNormals != null)
                {
                    this._normals.Add(stripNormals[i]);
                }

                if (this._textureCoordinates != null && stripTextureCoordinates != null)
                {
                    this._textureCoordinates.Add(stripTextureCoordinates[i]);
                }
            }
        ...
        public void AddPolygon(IList<int> vertexIndices)
        {
            ...
            for (int i = 0; i + 2 < n; i++)
            {
                this._triangleIndices.Add(vertexIndices[0]);
                this._triangleIndices.Add(vertexIndices[i + 1]);
                this._triangleIndices.Add(vertexIndices[i + 2]);
            }
        }
        ...
        {
            var points = vertexIndices.Select(vi => this._positions[vi]).ToList();
            ...
            if (triangulatedIndices != null)
            {
                ...
                {
                    this._triangleIndices.Add(vertexIndices[i]);
                }
            }
        }
            ...
            IList<Point> triangleTextureCoordinates = null)
        {
            ...
            if (this._normals != null && triangleNormals == null)
            {
                throw new ArgumentNullException("triangleNormals");
            }

            if (triangleTextureCoordinates == null) this._textureCoordinates = null;

            if (this._textureCoordinates != null && triangleTextureCoordinates == null)
            {
        ...
        public void AddTriangles(
         List<Point3D> trianglePositions,
         List<Vector3D> triangleNormals,
         List<Point> triangleTextureCoordinates, bool freeze)
        {
            if (trianglePositions == null)
            {
                throw new ArgumentNullException("trianglePositions");
        ...
        public void Append(MeshBuilder mesh)
        {
            ...
            this.Append(mesh._positions, mesh._triangleIndices, mesh._normals, mesh._textureCoordinates);
        }
        ...
        public void Append(MeshGeometry3D mesh)
        {
            ...
            this.Append(mesh.Positions, mesh.TriangleIndices, this._normals != null ? mesh.Normals : null, this._textureCoordinates != null ? mesh.TextureCoordinates : null);
        }
            ...
            IList<Point> textureCoordinatesToAppend = null)
        {
            ...
            if (this._normals != null && normalsToAppend == null)
            {
                throw new InvalidOperationException(SourceMeshNormalsShouldNotBeNull);
            }

            if (this._textureCoordinates != null && textureCoordinatesToAppend == null)
            {
        ...
        public void ChamferCorner(Point3D p, double d, double eps = 1e-6, IList<Point3D> chamferPoints = null)
        {
            ...
            int index0 = this._positions.Count;
            this._positions.Add(newCornerPoint);

            var plane = new Plane3D(newCornerPoint, cornerNormal);

            int ntri = this._triangleIndices.Count;
            ...
            for (int i = 0; i < ntri; i += 3)
            {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                var p0 = this._positions[this._triangleIndices[i0]];
                var p1 = this._positions[this._triangleIndices[i1]];
                var p2 = this._positions[this._triangleIndices[i2]];
                ...
                p0 = this._positions[this._triangleIndices[i0]];
                p1 = this._positions[this._triangleIndices[i1]];
                p2 = this._positions[this._triangleIndices[i2]];
                ...
                this._positions[this._triangleIndices[i01]] = p01.Value;

                int i02 = this._positions.Count;
                this._positions.Add(p02.Value);

                // add a new triangle for the other chamfer point
                this._triangleIndices.Add(i01);
                this._triangleIndices.Add(i2);
                this._triangleIndices.Add(i02);
        ...
        {
            if (this._positions.Count > 20000)
            {
                Trace.WriteLine(string.Format("Too many positions ({0}).", this._positions.Count));
            }

            if (this._triangleIndices.Count > 60002)
            {
                Trace.WriteLine(string.Format("Too many triangle indices ({0}).", this._triangleIndices.Count));
            }
        }
        ...
        {
            if (this._triangleIndices.Count == 0)
            {
            ...
            if (this._normals != null && this._positions.Count != this._normals.Count)
            {
                throw new InvalidOperationException(WrongNumberOfNormals);
            }

            if (this._textureCoordinates != null && this._positions.Count != this._textureCoordinates.Count)
            {
                throw new InvalidOperationException(WrongNumberOfTextureCoordinates);
            }
            ...
            {
                Positions = new Point3DCollection(this._positions),
                TriangleIndices = new Int32Collection(this._triangleIndices)
            };
            if (this._normals != null)
            {
                mg.Normals = new Vector3DCollection(this._normals);
            }

            if (this._textureCoordinates != null)
            {
                mg.TextureCoordinates = new PointCollection(this._textureCoordinates);
            }
        ...
        private static MeshGeometry3D GetUnitSphere(int subdivisions)
        {
            ...
            for (int i = 0; i < mb._positions.Count; i++)
            {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    ...
                    var u = Point3D.Subtract(
                        this._positions[index0 + (i1 * columns) + j0], this._positions[index0 + (i0 * columns) + j0]);
                    var v = Point3D.Subtract(
                        this._positions[index0 + (i0 * columns) + j1], this._positions[index0 + (i0 * columns) + j0]);
        ...
        private void AddRectangularMeshTextureCoordinates(int rows, int columns)
        {
            for (int i = 0; i < rows; i++)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    ...
                    this._textureCoordinates.Add(new Point(u, v));
                }
            }
        }
        ...
        private void AddRectangularMeshTriangleIndices(int index0, int rows, int columns, bool isSpherical = false)
        {
            for (int i = 0; i < rows - 1; i++)
            {
                for (int j = 0; j < columns - 1; j++)
                {
                    ...
                    if (!isSpherical || i > 0)
                    {
                        this._triangleIndices.Add(index0 + ij);
                        this._triangleIndices.Add(index0 + ij + 1 + columns);
                        this._triangleIndices.Add(index0 + ij + 1);
                    }
                    ...
                    if (!isSpherical || i < rows - 2)
                    {
                        this._triangleIndices.Add(index0 + ij + 1 + columns);
                        this._triangleIndices.Add(index0 + ij);
                        this._triangleIndices.Add(index0 + ij + columns);
                    }
                }
            }
        }
            ...
            int index0, int rows, int columns, bool rowsClosed, bool columnsClosed)
        {
            ...
            for (int i = 0; i < m2; i++)
            {
                for (int j = 0; j < n2; j++)
                {
                    ...
                    this._triangleIndices.Add(i00);
                    this._triangleIndices.Add(i11);
                    this._triangleIndices.Add(i01);

                    this._triangleIndices.Add(i11);
                    this._triangleIndices.Add(i00);
                    this._triangleIndices.Add(i10);
                }
            }
        }
        ...
        private Vector3D FindCornerNormal(Point3D p, double eps)
        {
            ...
            for (int i = 0; i < this._triangleIndices.Count; i += 3)
            {
                int i0 = i;
                int i1 = i + 1;
                int i2 = i + 2;
        ...
        private void NoSharedVertices()
        {
            ...
            for (int i = 0; i < this._triangleIndices.Count; i += 3)
            {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                int index0 = this._triangleIndices[i0];
                int index1 = this._triangleIndices[i1];
                int index2 = this._triangleIndices[i2];
                var p0 = this._positions[index0];
                var p1 = this._positions[index1];
                var p2 = this._positions[index2];
                p.Add(p0);
                p.Add(p1);
                p.Add(p2);
                ti.Add(i0);
                ...
                if (n != null)
                {
                    n.Add(this._normals[index0]);
                    n.Add(this._normals[index1]);
                    n.Add(this._normals[index2]);
                }

                if (tc != null)
                {
                    tc.Add(this._textureCoordinates[index0]);
                    tc.Add(this._textureCoordinates[index1]);
                    tc.Add(this._textureCoordinates[index2]);
                }
            }

            this._positions = p;
            this._triangleIndices = ti;
            this._normals = n;
            this._textureCoordinates = tc;
        }
