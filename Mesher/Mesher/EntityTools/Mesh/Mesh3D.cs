...
namespace KneeInnovation3D.EntityTools
{
    ...
    public class Mesh3D : ICloneable
    {
        ...
        public static void Transform(MeshGeometry3D  mesh, Transform3D transform)
        {
        ...
        public static void Scale(MeshGeometry3D mesh, double scaleX, double scaleY, double scaleZ)
        {
            for (int i = 0; i < mesh.Positions.Count; i++)
            {
                mesh.Positions[i] = new Point3D(
                    mesh.Positions[i].X * scaleX, mesh.Positions[i].Y * scaleY, mesh.Positions[i].Z * scaleZ);
            }
        }

        public static MeshGeometry3D Append ( MeshGeometry3D inputModel1,  MeshGeometry3D inputModel2)
        {
            int initialCount = inputModel1.Positions.Count;
            for (int p = 0; p < inputModel2.Positions.Count; p++)
            {
                inputModel1.Positions.Add(inputModel2.Positions[p]);
            }
            for (int i = 0; i < inputModel2.TriangleIndices.Count; i++)
            {
                inputModel1.TriangleIndices.Add(initialCount + inputModel2.TriangleIndices[i]);
            }
