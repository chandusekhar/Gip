...
namespace ViewPortTools
{
    ...
    public class ScreenSpaceLines3D : ModelVisual3D
    {
        ...
        {
            if (VisualTreeHelper.GetParent(this) == null && oldParent != null && _rendered )
            {
                CompositionTarget.Rendering -= OnRender;
                _rendered  = false;
            }
            else if (VisualTreeHelper.GetParent(this) != null && oldParent == null && !_rendered )
            {
                CompositionTarget.Rendering += OnRender;
                _rendered  = true;
            }
        ...
        {
            ((ScreenSpaceLines3D)sender).SetMaterial((Material)args.NewValue);
        }
