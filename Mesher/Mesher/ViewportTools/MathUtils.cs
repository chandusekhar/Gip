...
namespace ViewPortTools
{
    public static class MathUtils
    {
        ...
        public static Matrix3D TryWorldToCameraTransform(Viewport3DVisual visual, out bool success)
        {
            ...
            if (visual == null) return ZeroMatrix;
        ...
        public static Transform3D TransformFromCordSys(Point3D origin, Vector3D zaxis)
        {
            Transform3D invTr = TransformtoCordSys(origin, zaxis);
            return (Transform3D ) invTr.Inverse;
        }

        public static Transform3D TransformFromCordSys(Point3D origin, Vector3D zaxis, Vector3D yaxis , Vector3D xaxis)
        {
            Transform3D invTr = TransformtoCordSys(origin, xaxis, yaxis,   zaxis);
            return (Transform3D)invTr.Inverse;
        }

        public static Transform3D GroupTransform(params Transform3D[] inputransforms)
        {
            Transform3DGroup transformgroup = new Transform3DGroup();

            foreach (var v in inputransforms)
                transformgroup.Children.Add((Transform3D)v);
        ...
        /// <summary>
        /// Gets the line line intersecion.
        /// </summary>
        ...
        public static bool GetLineLineIntersection(Point Start1, Point End1, Point Start2, Point End2, out Point IntersectionPoint)
        {
           double M = (End1.Y - Start1.Y) / (End1.X - Start1.X);

           if (double.IsInfinity(M)) M = 1;

           double A = -M;
           double B = 1;
           double C = Start1.Y - M * Start1.X;
        ...
        public static bool FindIntersection( Point Start1, Point End1, Point Start2, Point End2, out bool segments_intersect,out Point intersection, out Point close_p1, out Point close_p2)
        {
            // Get the segments' parameters.
            double dx12 = End1.X - Start1.X;
            double dy12 = End1.Y - Start1.Y;
            double dx34 = End2.X - Start2.X;
            double dy34 = End2.Y - Start2.Y;

            // Solve for t1 and t2
            double denominator = (dy12 * dx34 - dx12 * dy34);

            double t1 =
                ((Start1.X - Start2.X) * dy34 + (Start2.Y - Start1.Y) * dx34)
                    / denominator;
            ...
            double t2 =
                ((Start2.X - Start1.X) * dy12 + (Start1.Y - Start2.Y) * dx12)
                    / -denominator;

            // Find the point of intersection.
            intersection = new Point(Start1.X + dx12 * t1, Start1.Y + dy12 * t1);
            ...
            close_p1 = new Point(Start1.X + dx12 * t1, Start1.Y + dy12 * t1);
            close_p2 = new Point(Start2.X + dx34 * t2, Start2.Y + dy34 * t2);
