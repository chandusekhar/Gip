...
namespace ViewPortTools
{
    ...
    public class Interactive3DDecorator : Viewport3DDecorator
    {
        ...
        private void FindClosestIntersection(Point mousePos, List<HitTestEdge> edges, InteractiveVisual3D imv3DHit)
        {
            ...
            {
                Vector v1 = mousePos - edges[i].P1Transformed;
                Vector v2 = edges[i].P2Transformed - edges[i].P1Transformed;
                ...
                if (denom == 0)
                {
                    currClosest = edges[i].P1Transformed;
                    distance = v1.Length;
                }
                else
                {
                    ...
                    else
                    {
                        if (numer > denom)
                        {
                            currClosest = edges[i].P2Transformed;
                        }
                        else
                        {
                            currClosest = edges[i].P1Transformed + (numer / denom) * v2;
                        }
                    }
                ...
                if (distance < closestDistance)
                {
                    ...
                    if (denom != 0)
                    {
                        closestIntersection = ((currClosest - edges[i].P1Transformed).Length / Math.Sqrt(denom) * 
                                               (edges[i].Uv2 - edges[i].Uv1)) + edges[i].Uv1;
                    }
                    else
                    {
                        closestIntersection = edges[i].Uv1;
                    }
                }                
            }
            ...
            if (closestDistance != Double.MaxValue)
            {
                ...
                if (ptRelToCapture.X <= contBounds.Left + 1) ptRelToCapture.X -= BufferSize;
                if (ptRelToCapture.Y <= contBounds.Top + 1) ptRelToCapture.Y -= BufferSize;
                if (ptRelToCapture.X >= contBounds.Right - 1) ptRelToCapture.X += BufferSize;
                if (ptRelToCapture.Y >= contBounds.Bottom - 1) ptRelToCapture.Y += BufferSize;
        ...
        private bool UpdateHiddenVisual(ClosestIntersectionInfo isectInfo, Point mousePos, bool scaleHiddenVisual)
        {
            ...
            if (isectInfo != null)
            {
                ...
                if (_hiddenVisual.Child != uiElem)
                {
                    ...
                    Decorator oldVisToUse = null;
                    if (prevVisual != null && prevVisual.IsKeyboardFocusWithin)
                    {
                        oldVisToUse = _oldKeyboardFocusVisual;
                    }
                    else
                    {
                        oldVisToUse = _oldHiddenVisual;
                    }

                    // now safely link everything up
                    _hiddenVisual.Child = uiElem;
                    oldVisToUse.Child = prevVisual;
        ...
        public class DebugEdgesAdorner : Adorner
        {
            ...
            protected override void OnRender(DrawingContext drawingContext)
            {
                ...
                {
                    drawingContext.DrawLine(renderPen, _edges[i].P1Transformed, _edges[i].P2Transformed);
                }
        ...
        private const double BufferSize = 2.0;  // the "ring" around the element with capture to use in the capture case        
    }
}
